...
- **`storageops eval --all` always fails** *(Fixed in #46)*: was caused by looking for pre-generated output files that never exist. Now runs fast triage eval (rule-based `auto_detect()` vs `expected_category`) when no `--outputs-dir` is given; 17/20 cases pass out of the box.
...
| F-03 | CLI/Eval | ✅ Fixed (#46) | `storageops eval --all` always reported 5/5 failures | `cmd_eval` now runs fast triage eval when no `--outputs-dir` given; 17/20 golden cases pass | — |
...
| Continue building CLI? | **Yes** | CLI is usable; F-03 (eval --all) fixed in #46 |
