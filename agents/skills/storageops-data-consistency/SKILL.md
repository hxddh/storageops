...
|---|---|---|
| `parse_replication_status` | When replication status metadata or inventory is provided | `{"text": "<replication status or inventory export>"}` |
| `analyze_replication` | After parsing, to identify the consistency gap | `{"text": "<parsed replication evidence>"}` |
| `scan_secrets` | Before any output, redact credentials from evidence | `{"text": "<log or config content>"}` |
| `search_memory` | At start, check for prior consistency cases | `{"query": "data consistency stale replica <provider>"}` |
